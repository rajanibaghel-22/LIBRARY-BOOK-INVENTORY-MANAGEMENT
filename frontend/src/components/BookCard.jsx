...
const BookCard = ({ book, setBooks }) => {
  ...
  return (
    <>
      ...
      <Link
        to={`/book/${book._id}`}
        className={`relative block rounded-xl bg-base-100 p-4 border transition-all duration-200 ${isActive ? "border-primary shadow-lg" : "border-base-300"} hover:border-primary hover:shadow-xl`}
      >
        {/* Top Row */}
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex items-center gap-2 text-base-content/70">
            ...
            <p className="text-sm line-clamp-1"> {book.category} </p>
          </div>
          ...
          <div className="flex items-center gap-2 text-base-content/70">
            ...
            <p className="text-sm line-clamp-1"> {book.price} </p>
          </div>
          ...
          <div className="flex items-center gap-2 text-base-content/70">
            ...
            <p className="text-sm line-clamp-1">
              {formatData(new Date(book.purchased_on))}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-between items-center">
          <span className="text-xs text-base-content/60">
            {formatData(new Date(book.updatedAt))}
          </span>
